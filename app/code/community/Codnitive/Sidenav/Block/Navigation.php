...
class Codnitive_Sidenav_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    protected function _renderCategoryMenuItemHtml(
        ...
        $count = Mage::getModel('catalog/layer')->setCurrentCategory($category->getID());

        $alt_url = $count->getCurrentCategory()->getAlternativeUrl();

        if(isset($alt_url)) {      

            $url = $count->getCurrentCategory()->getAlternativeUrl();
        ...
        $html[] = '<a ' . $aClass . $onclick . 'href="' . $url . '"'
